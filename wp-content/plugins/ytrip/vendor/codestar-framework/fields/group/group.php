...
if ( ! class_exists( 'CSF_Field_group' ) ) {
  class CSF_Field_group extends CSF_Fields {
    ...
    public function render() {
      ...
      // FIX: Ensure fields is always an array
if ( ! isset( $this->field['fields'] ) || ! is_array( $this->field['fields'] ) ) {
  $this->field['fields'] = array();
}

// FIX: If fields array is empty, show a message
if ( empty( $this->field['fields'] ) ) {
  echo '<div class="csf-notice csf-notice-warning">';
  echo esc_html__( 'No sub-fields defined for this group. Please configure fields in the admin panel.', 'csf' );
  echo '</div>';
  return;
}
      ...
      if ( preg_match( '/'. preg_quote( '['. $this->field['id'] .']' ) .'/', $this->unique ) ) {
        ...
        echo '<div class="csf-cloneable-item csf-cloneable-hidden" data-depend-id="'. esc_attr( $this->field['id'] ) .'">';
          ...
          // FIX: Only loop if fields is array and not empty
if ( is_array( $this->field['fields'] ) && ! empty( $this->field['fields'] ) ) {
